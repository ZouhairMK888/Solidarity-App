  ...
  try {
    ...
    if (!['admin', 'organizer'].includes(req.user.role)) {
      return res.status(403).json({
        success: false,
        message: 'Only admins and organizers can review mission applications.',
      });
    }

    const applications = await VolunteerApplicationModel.findManageable({ status, user: req.user });
...
const reviewMissionApplication = async (req, res, next) => {
  ...
  try {
    ...
    if (!await CampaignModel.canUserManage(campaign, req.user)) {
      return res.status(403).json({
        success: false,
        message: 'You can only review mission applications for campaigns assigned to you.',
      });
    }
